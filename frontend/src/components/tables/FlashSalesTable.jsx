...
const FlashSalesTable = () => {
  ...
  const fetchFlashSales = async () => {
    try {
      ...
      const formattedData = response.data.data.map((item) => ({
        ...
        products: [], // Optional, for future use
      }));

      setFlashSales(formattedData);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await fetchFlashSales(); // ดึงข้อมูลใหม่หลังลบ
    ...
    <div className="w-full mx-auto flex flex-col gap-2">
      <div className="border border-slate-200 rounded-md overflow-hidden font-satoshi">
        <div className="max-w-full overflow-x-auto">
          {loading ? (
            <div className="text-center py-10 text-slate-500">Loading promotions...</div>
          ) : (
                ...
                {flashSales.length > 0 ? (
                  flashSales.map((sale) => (
                    <TableRow key={sale.id} className="border-t border-slate-200">
                      ...
                      <TableCell className="px-7 py-3 text-sm text-slate-500">{sale.promotionName}</TableCell>
